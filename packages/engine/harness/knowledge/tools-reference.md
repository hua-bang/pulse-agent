...
- **grep** — runs `rg` via async `execFile` with an argument array (no shell, non-blocking; previously `execSync` with a built shell string — that had a command-injection hole and blocked the event loop, fixed with a regression test). `outputMode` files_with_matches/count/content, `-i`, `-U`, glob/type filters; `offset/headLimit` applied in-process. rg exit 1 = "(no matches found)"; other failures throw with stderr.
