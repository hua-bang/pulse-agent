...
- **grep** — runs `rg` via async `execFile` with an argument array (no shell, non-blocking; previously `execSync` (`shell:/bin/bash`) with a built shell string whose arguments were quoted only when they contained space/`$`/`*`, so a pattern like `x;touch /tmp/pwned` reached the shell unescaped — command injection plus event-loop blocking; `src/tools/grep.test.ts` pins that the injecting pattern does not execute). `outputMode` files_with_matches/count/content, `-i`, `-U`, glob/type filters; `offset/headLimit` applied in-process. rg exit 1 = "(no matches found)"; other failures throw with stderr.
- **ls** — `path?` (default `.`). Plain `readdirSync`, names only, no recursion/stats. Dedup-wrapped by the loop.
- **bash** — `command`, `timeout?` (default 120 000 ms, max 600 000, out-of-range throws), `cwd?`. Async `spawn` on `/bin/bash`; stdout/stderr capped at 10 MB each (pipe keeps draining, excess dropped with a truncation note). On POSIX the shell owns a detached process group, so timeout or `abortSignal` sends `SIGTERM` and then `SIGKILL` after 2 000 ms to the full descendant tree; Windows falls back to the direct child. Never throws for command failure — reports `{output, error?, exitCode}`.
...
- **No `execSync`/blocking I/O anywhere in `src/tools/*`** — the engine runs on GUI main threads (Electron hosts): the `bash` tool's original `execSync` froze the `canvas-workspace` UI outright. Two wrong-root-cause fixes (a pulse-sandbox interrupt, PTY coalescing) were reverted before the real blocking call was found — confirm the actual blocking call before patching adjacent paths. From the same async rewrite: decode collected raw `Buffer`s once at the end, never per pipe chunk — per-chunk decoding corrupted multi-byte CJK output.
