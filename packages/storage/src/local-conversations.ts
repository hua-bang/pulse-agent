...
export async function openLocalConversationStorage(options: LocalStorageOptions): Promise<PulseStorage | null> {
  const status = await readLocalStorageStatus(options.root);
  return status?.domains.includes('conversations') ? openLocalStorage(options) : null;
}
  ...
  try {
    return await withLegacyCanvasWrite(options.root, async () => {
      ...
      const storage = await openLocalStorage(options) ?? await openSqliteStorage({
        path: join(options.root, '__storage__.sqlite'), nativeBinding: options.nativeBinding,
      });
