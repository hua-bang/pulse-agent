...
- `@` references expand at submit time in BOTH hosts (`runMessage` Ink, `executeAgentTurn` readline): raw text stays, contents append below; bounded (bytes/count/entries), refuses binaries + workspace escapes. Image refs are vision input via `buildUserContent` (5MB cap). `/paste-image` reads the clipboard image through the same channel. Detail: `harness/knowledge/file-references.md`.
