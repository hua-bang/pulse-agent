...
| Workspace-local validation | Workspace `validation.yaml` if present, otherwise workspace `docs/validation.md` |
...
`harness/validate/validation.yaml` is declarative. Nothing runs it today — there is no CI, no git hooks, no husky/lint-staged, and no executable harness checks yet. Validation is currently carried by agent discipline plus manual `pnpm --filter` runs. State this honestly; do not claim a gate exists when it does not.
...
- Workspace-specific validation details: keep in workspace `validation.yaml` or `docs/validation.md`.

## When to add files

Add a workspace `validation.yaml` when that workspace needs machine-readable local validation. Keep root `harness/validate/validation.yaml` for global config changes and cross-workspace impact rules.
