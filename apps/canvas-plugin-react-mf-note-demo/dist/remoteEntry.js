import { _ as __vitePreload } from "./assets/preload-helper-CmrIWoG2.js";
...
function generateSnapshotFromManifest(manifest, options = {}) {
  ...
  let remotesInfo = {};
  if (!Object.keys(remotes).length) remotesInfo = ((_a = manifest.remotes) == null ? void 0 : _a.reduce((res, next) => {
    let matchedVersion;
...
function setGlobalFederationConstructor(FederationConstructor, isDebug = isDebugMode()) {
  if (isDebug) {
    ...
    CurrentGlobal.__FEDERATION__.__DEBUG_CONSTRUCTOR_VERSION__ = "2.5.1";
  }
}
...
function formatShare(shareArgs, from, name, shareStrategy) {
  ...
  if (((_a = shareArgs.shareConfig) == null ? void 0 : _a.eager) && ((_b = shareArgs.treeShaking) == null ? void 0 : _b.mode)) error(`Invalid shared config for "${name}" from "${from}": cannot use both "eager: true" and "treeShaking.mode" simultaneously. Choose one strategy.`);
  return {
    deps: [],
    useIn: [],
    from,
  ...
  emit(...data) {
    ...
    if (ls.length > 0) {
      ...
      const call = (prev) => {
        ...
        else if (i < ls.length) return Promise.resolve(ls[i++].apply(null, data)).then((result2) => {
          if (result2 === void 0 || data.length === 1 && result2 === data[0]) return call(prev);
          return call(result2);
        });
        else return prev;
      };
...
var PluginSystem = class {
  ...
  applyPlugin(plugin, instance) {
    ...
    if (!this.registerPlugins[pluginName]) {
      ...
      (_a = plugin.apply) == null ? void 0 : _a.call(plugin, instance);
...
function generatePreloadAssets(origin, preloadOptions, remote, globalSnapshot, remoteSnapshot) {
  ...
  if (remoteSnapshot.shared && remoteSnapshot.shared.length > 0) {
    ...
    remoteSnapshot.shared.forEach((shared) => {
      var _a;
      const shareInfos = (_a = options.shared) == null ? void 0 : _a[shared.sharedName];
...
var SharedHandler = class {
  ...
  emitErrorLoadShare({ lifecycle, pkgName, shareInfo, error: error2, recovered }) {
    try {
    ...
    try {
      ...
      if (registeredShared) {
        ...
        if (targetShared.lib) {
          ...
          const asyncLoadProcess = async () => {
            const factory2 = await targetShared.get();
            addUseIn(targetShared, host.options.name);
            targetShared.loaded = true;
            targetShared.lib = factory2;
            return factory2;
          };
  ...
  */
  initializeSharing(shareScopeName2 = DEFAULT_SCOPE, extraOptions) {
    const { host } = this;
    ...
    Object.keys(host.options.shared).forEach((shareName) => {
      host.options.shared[shareName].forEach((shared) => {
        if (shared.scope.includes(shareScopeName2)) register(shareName, shared);
      });
    });
    if (host.options.shareStrategy === "version-first" || strategy === "version-first") host.options.remotes.forEach((remote) => {
      if (remote.shareScope === shareScopeName2) promises.push(initRemoteModule(remote.name));
    });
    return promises;
  }
  loadShareSync(pkgName, extraOptions) {
    ...
    try {
      ...
      if (shareOptions.get) {
        ...
        if (module instanceof Promise) error((extraOptions == null ? void 0 : extraOptions.from) === "build" ? RUNTIME_005 : RUNTIME_006, runtimeDescMap, {
...
var RemoteHandler = class {
  ...
  async loadRemote(id, options) {
    ...
    let completeExpose = startMatchInfo == null ? void 0 : startMatchInfo.expose;
  ...
  registerRemotes(remotes, options) {
    ...
    remotes.forEach((remote) => {
      this.registerRemote(remote, host.options.remotes, { force: options == null ? void 0 : options.force });
    });
  }
    ...
    const normalizeRemote = () => {
      if (remote.alias) {
        const findEqual = targetRemotes.find((item) => {
          var _a;
          return remote.alias && (item.name.startsWith(remote.alias) || ((_a = item.alias) == null ? void 0 : _a.startsWith(remote.alias)));
        });
  ...
  removeRemote(remote) {
    ...
    try {
      ...
      if (loadedModule) {
        ...
        if (CurrentGlobal[key]) if ((_a = Object.getOwnPropertyDescriptor(CurrentGlobal, key)) == null ? void 0 : _a.configurable) delete CurrentGlobal[key];
...
const __mfResolveGlobalKey = "__mf_init__virtual:mf:__mfe_internal__pulse_canvas_demo_note__mf_v__runtimeInit__mf_v__.js__";
...
const mfName = "pulse_canvas_demo_note";
...
async function init(shared = {}, initScope = []) {
  ...
  try {
    const allInstances = (_a = globalThis.__FEDERATION__) == null ? void 0 : _a.__SHARE__;
    if (allInstances) {
      ...
      for (const [, scopes] of Object.entries(allInstances)) {
        const scopeShare = scopes == null ? void 0 : scopes["default"];
        ...
        for (const [pkg, versionMap] of Object.entries(scopeShare)) {
          for (const [version, provider] of Object.entries(versionMap)) {
            ...
            const cacheKey = ((_b = provider.shareConfig) == null ? void 0 : _b.singleton) ? pkg : `${pkg}@${version}`;
  ...
  for (const [pkg, share] of Object.entries(usedShared)) {
    const cacheKey = ((_c = share.shareConfig) == null ? void 0 : _c.singleton) || !share.version ? pkg : `${pkg}@${share.version}`;
    if (((_d = share.shareConfig) == null ? void 0 : _d.import) !== false || __mfModuleCache.share[cacheKey] !== void 0) continue;
    const __mfNormalizeRuntimeShare = (mod2) => {
      let current = mod2;
      for (let i = 0; i < 5; i++) {
        const defaultExport = current == null ? void 0 : current.default;
    ...
    const versions = shared == null ? void 0 : shared[pkg];
    const provider = versions && versions[Object.keys(versions)[0]];
    if (!provider) continue;
    const factory = provider.lib || (provider.loading ? await provider.loading : await ((_e = provider.get) == null ? void 0 : _e.call(provider)));
