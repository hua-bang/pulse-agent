...
| Periodic task scheduler | `src/main/scheduler/task-scheduler.ts` — generic weekly/monthly/interval tasks with persisted lastRun + startup catch-up; deliberately not cron (no expressions/queues/retries; a failing run consumes its period; a first-seen task is ANCHORED, not run — features wanting "see one now" ship an explicit user action instead, e.g. `agent/memory-report-ipc.ts`'s `memory-report:run-now` behind the settings try-it button, registered unconditionally since a click is explicit consent). Registered tasks live in `app/bootstrap.ts`; the only current task (memory-report, weekly) is gated behind the `scheduled-memory-report` experimental flag, default off — flag off means the scheduler is never constructed. Tests: `src/main/scheduler/__tests__/task-scheduler.test.ts` |
