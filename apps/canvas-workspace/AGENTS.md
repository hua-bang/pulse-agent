...
| Scheduled tasks | `src/main/scheduled/` — stable top-level Scheduled surface with persisted user-defined interval tasks (minimum 30 minutes), an exact next-due main-process timer backed by a 30-minute heartbeat, startup/resume catch-up, manual run-now, and one isolated durable Agent chat scope per task. Failed attempts consume the current interval rather than hot-looping. The built-in weekly memory-report prompt is seeded idempotently as a disabled Scheduled task; it is no longer an Experimental entry. IPC contract: `src/shared/scheduled.ts` → `src/preload/bridge/scheduled.ts` → renderer `components/Scheduled/`. Tests: `src/main/__tests__/scheduled-task-service.test.ts` plus scheduled-scope coverage in `src/main/agent/__tests__/service-history.test.ts`. |
