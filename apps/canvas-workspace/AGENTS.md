...
| Scheduled tasks | `src/main/scheduled/` — stable top-level Scheduled surface with persisted user-defined interval tasks (minimum 30 minutes), a 30-minute main-process due-task sweep, startup/resume catch-up, manual run-now, and one isolated durable Agent chat scope per task. Failed attempts consume the current interval rather than hot-looping. The built-in weekly memory-report prompt is seeded idempotently as a disabled Scheduled task; it is no longer an Experimental entry. IPC contract: `src/shared/scheduled.ts` → `src/preload/bridge/scheduled.ts` → renderer `components/Scheduled/`. Tests: `src/main/scheduled/__tests__/scheduled-task-service.test.ts` plus scheduled-scope coverage in `src/main/agent/__tests__/service-history.test.ts`. |
