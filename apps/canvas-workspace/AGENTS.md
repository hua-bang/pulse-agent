...
| Dock web tabs (the embedded browser) | A `<webview>` guest is a SEPARATE WebContents, and every rule below follows from that. Keys pressed while a page has focus never reach this window, so browsing shortcuts live in `src/shared/dock-shortcuts.ts` (the one chord policy for both processes) and `main/app/webview-shortcuts.ts` relays them from `before-input-event`; `RightDock/dock-browser-commands.ts` maps a command to the store and broadcasts the per-tab ones (focus address / reload / find) as window events the ACTIVE `LinkTabView` claims. `DOCK_FOCUS_SCOPED_COMMANDS` is for chords the canvas also binds (⌘F) — listener order between two window handlers is not a contract, so those are claimed only when focus is already inside the dock. Clicks inside a guest do not reach the host document either, so any overlay above a page (address suggestions, the page context menu) must hold `useGuestInteractionShield` or `useClickOutside` never fires and it strands on top of the page. A guest's `context-menu` is likewise invisible here: `main/app/webview-context-menu.ts` forwards it and the owning tab draws `LinkDrawer/PageContextMenu.tsx`, using Electron's already-host-viewport-relative coordinates directly. Closing a web tab is the only destructive tab action (history, scroll, sign-in), so Escape collapses the dock rather than closing, ⌘W closes, and every close lands on `DockStore`'s bounded, workspace-scoped reopen stack (⌘⇧T) — including bulk closes from the tab menu. `link:open` carries the Chromium disposition and the source guest id: `background-tab` must not steal focus, and the id resolves through `RightDock/link-tab-webviews.ts` to place a new tab beside its opener. Favicons survive same-origin navigation (SPA pushState fires `did-navigate-in-page` without re-announcing the icon). Switching workspaces KEEPS the left workspace's web tabs mounted (`DockState.retainedLinkTabs`, bounded by `RETAINED_WORKSPACE_LIMIT`) so a canvas switch does not reload every page; three separate things are load-bearing and each one silently defeats retention on its own: `DockPanes` renders live and retained panes from ONE key-sorted list (React remounts across sibling arrays, and Chromium reloads a `<webview>` moved within its parent), the mounted-pane set is keyed off `state.activeTerminalWorkspaceId` rather than the `activeWorkspaceId` prop (the prop advances one render before the store does, and keying off it prunes every live key), and a hidden guest's navigations write back through `updateRetainedLinkTab` (a stale stored URL is treated as a navigation command on restore, yanking the page back). All three were found only by driving the real app. A menu anchored INSIDE the dock portals to `body` and therefore lands in a lower stacking context than `.right-dock` (`--layer-dock`) — it needs `context-menu--in-dock` (`--layer-dock-menu`) or it is painted behind the panel it describes while still being in the DOM, correctly positioned, and passing every unit test. Driving this surface with `harness/tools/driver/` has one hard limit worth knowing before you conclude a shortcut is broken: `harness press`/`click` dispatch CDP input to the HOST page target, which a guest never receives, so the `before-input-event` relay cannot be exercised that way — attach to the guest's own CDP target (it appears in `/json/list` as `type: webview`) and dispatch there. Tests: `RightDock/__tests__/dock-link-tabs.test.ts`, `dock-store.test.ts`, `TabContextMenu.test.tsx`, `dock-width.test.ts`, `LinkDrawer/__tests__/address-bar.test.tsx`, `find-in-page.test.tsx`, `page-context-menu.test.tsx`, `EmbeddedBrowser/load-error.test.ts`, `src/shared/dock-shortcuts.test.ts`, `main/app/__tests__/webview-shortcuts.test.ts`, `webview-context-menu.test.ts`, `link-policy.test.ts` |
