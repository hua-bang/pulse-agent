...
| Scheduled tasks | `src/main/scheduled/` — stable top-level Scheduled surface with persisted user-defined tasks, an exact next-due main-process timer backed by a 30-minute heartbeat, startup/resume catch-up, manual run-now, and one isolated durable Agent chat scope per task. Cadence is the `ScheduledSchedule` union in `src/shared/scheduled.ts`: `interval` (relative, minimum 30 minutes, anchored at create/enable/last-attempt) or `daily`/`weekly` at a LOCAL wall-clock `HH:mm`. `computeNextRunAt` is the single next-run authority for all three kinds — use local `Date` field arithmetic there, never fixed millisecond offsets, so absolute slots survive DST. A slot missed while the app was closed runs ONCE on catch-up and then realigns to the next slot (never one run per missed slot); failed attempts consume the current slot rather than hot-looping. Pre-`schedule` records carrying `intervalMinutes` are lifted into the union on read (`migratePersistedTask`); the field is gone from the live contract. The built-in weekly memory-report prompt is seeded idempotently as a disabled Scheduled task on `weekly` Monday 09:00 local; it is no longer an Experimental entry. Seeding is one-shot by design — an install that already carries the task keeps its stored schedule, so the Monday default reaches new installs only. A finished attempt — success AND failure — is announced by `announceRunFinished` (`scheduled/runtime.ts`) as a `scheduled:run-finished` push that `useScheduledRunToasts` turns into a STICKY toast (`autoCloseMs: 0`); its action opens the task's conversation in the DOCK's Pulse AI tab (`useScheduledRunChatOpener` → `dock.openScheduledChat`), the same surface `Run now` uses — acting on a finished run must never navigate the whole app onto the AI Chat page and lose what the user was looking at. Every push carries `trigger: 'manual' \| 'schedule'` (`runTask`'s single source for BOTH "does this run consume its slot" and "does it need announcing" — it replaced the former `advanceSchedule` flag, and `execute` receives it as `{ trigger }`): a SUCCESSFUL `manual` run whose conversation is already the visible dock chat raises no toast, because `Run now` opened that exact panel and held the user there for the whole run. Failures and every `schedule` run always toast. The "is it on screen" test is `isScheduledChatVisible` (`RightDock/dock-chat-state.ts` — expanded AND chat tab active AND pointed at the task; merely pointed is not visible), and it is INJECTED into `useScheduledRunToasts` by `useScheduledRunChatOpener` rather than read there, so the toast hook stays free of the dock provider. That same opener ALSO subscribes to the push independently to call `dock.refreshScheduledChat` — an already-open panel had no other way to learn a background run finished, so the running banner's "the result will appear here" promise ended with the banner vanishing and no new message (the manual `Run now` path was the only refresher). Two subscribers on one channel is intended and real (`preload/bridge/ipc.ts`'s `subscribe` adds an independent `ipcRenderer.on` per call); a test double that stores ONE callback silently exercises half the hook. `ScheduledPage`'s own run-now error branch must stay narrowed to `!ok && !task` — the only failure with no push is the IPC catch that fires before the attempt; widening it back reinstates two identical error toasts for one click. Routing to `/chat?scheduledTask=<id>` survives only as the fallback for views that hide the dock chat tab; `isDockChatTabEnabled` (`components/RightDock/dock-chat-availability.ts`) is the single predicate behind BOTH that fallback and the dock's `chatTabEnabled` prop, because a caller that assumes a dock chat tab where there is none swallows the open silently. That fallback MUST navigate through `navigateCanvasRoute` (`utils/canvasLinks.ts`), never wouter's `setLocation`: the hash router's `navigate` splits its argument at the first `?` and writes the query to `location.search`, while `currentHashLocation()` reads back only the hash — so `setLocation('/chat?scheduledTask=…')` reached `App`'s `parseCanvasLocation` with empty params and the toast action did nothing at all on the AI Chat route. Any future in-app route carrying a query has the same constraint; assert the round trip (`utils/canvasLinks.test.ts`), because a resolver test that only checks the returned string passes while the navigation is broken. The same module derives `isGlobalChatLauncherVisible` — the floating Pulse-logo launcher (`RightDock/GlobalChatLauncher.tsx`) shows on every route that has a dock chat tab and no chat chrome of its own, canvas being the one exception; deriving it stopped the Scheduled page from being hand-excluded with no way to reach the agent (`__tests__/dock-chat-availability.test.ts`). `scheduledChatTaskId` is a PERSISTENT dock override, and only `getOpenChatPatch` clears it — re-clicking the (generically labelled) Pulse AI tab just re-selects the same task chat, so the conversation needs its own chrome: `ScheduledChatPanel` always renders an identity strip naming the task and its cadence plus a one-press `onExitTaskChat` (wired to `dock.openChat`), with the running/error status hanging under it rather than replacing it. Do not make that strip conditional on a run being in flight — "which conversation am I in, and how do I leave" is the idle question. It reuses `scheduled.automationLabel`/`automationCadence`, which existed unused for exactly this. Each task's chat is a session STORE (`__scheduled__-<taskId>`), listed in that rail beside workspaces and global chat — `src/shared/agent-chat.ts` owns the store-id vocabulary (`scopeSessionStoreId`, `scheduledTaskIdFromStoreId`, `isListableSessionStore`), and every consumer that maps a listed session back to a scope MUST go through it: a sentinel store id treated as a workspace id activates an agent against a workspace that does not exist. `__`-prefixed stores are allowlisted, never blanket-skipped (that is what hid scheduled chats from the rail). Deliberately in-app only: OS `Notification` was tried and removed (Focus modes, missing notification daemons, unsigned dev builds, and Windows-without-AppUserModelID all drop it silently, and it needed a retained-reference dance to survive GC), so do not reintroduce a second channel — `scheduled-run-notify.test.ts` asserts none is raised. A run finishes while nobody is watching, so the toast must never expire on a timer. Writing a schedule RE-ANCHORS the next run, so a write must mean a real change: `updateTask` recomputes `nextRunAt` only when `isSameSchedule` (`src/shared/scheduled.ts`) says the normalized cadence differs, and only when `enabled` goes false→true. The editor submits the whole form, so `ScheduledPage`'s `changedFields` reduces an edit to just the touched keys before calling `update` — matching what `agent/tools/scheduled.ts` already builds. Without both halves a title typo fix pushed the pending run a full period out and could swallow a catch-up run whose overdue slot was still waiting. When a test needs to exercise re-anchor arithmetic it must therefore pass a genuinely DIFFERENT schedule. IPC contract: `src/shared/scheduled.ts` → `src/preload/bridge/scheduled.ts` → renderer `components/Scheduled/`; list rows are presentational — every action is an explicit button, and the time picker is hour/minute `ui/Select`s, never a native `<input type="time">`. The row is the only LASTING record of an outcome (the toast is dismissible and does not survive a restart), so failure has to be legible there: `--error` tone plus the `lastError` reason, and the status dot carries a failed tone too. The narrow and dock-expanded layouts drop the outcome column to fit, but must keep `.scheduled-page__last-run--error` (it wraps to its own grid row) — a dock open is the normal state right after opening a task's chat, which is exactly when the user is checking whether it worked. `scheduled.lastSuccess` says "Last run", NOT "Completed": both it and `scheduled.lastFailed` render an attempt's START (`lastSuccessAt` is set to `attemptedAt`, captured before the run), so "Completed" was falsifiable by watching a run take minutes. Storing a real finish time instead is the other valid fix, but it would break the slot alignment with the "Next …" line beside it. Chat entry: `scheduled_task_list`/`_create`/`_update` in `src/main/agent/tools/scheduled.ts` — app-level, so registered UNWRAPPED on both tool factories, all `defer_loading`, no delete (see `harness/knowledge/security-posture.md` for why); it dynamic-imports `scheduled/runtime` to avoid the tools→runtime→agent-service module cycle. Tests: `src/shared/scheduled.test.ts` (schedule validation + next-run math), `src/main/__tests__/scheduled-task-service.test.ts`, `src/main/__tests__/scheduled-run-notify.test.ts` (completion push, success AND failure, and no OS notification), `components/Scheduled/__tests__/useScheduledRunToasts.test.tsx` (sticky toast), `components/Scheduled/__tests__/scheduledChatTarget.test.ts` (dock-by-default vs route fallback), `src/main/agent/__tests__/scheduled-tools.test.ts`, `components/Scheduled/__tests__/TaskEditorModal.test.tsx` + `ScheduledPage.test.tsx`, plus scheduled-scope coverage in `src/main/agent/__tests__/service-history.test.ts`. |
