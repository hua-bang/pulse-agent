...
| AI Chat loading states | Read `harness/knowledge/chat-sessions.md` before changing session loading flags (`sessionsLoading` vs `sessionLoading` vs stream `loading`) or the submit veto. Key contracts: `src/renderer/src/agent-chat/sessions/useChatSessions.ts`, `components/chat/hooks/useMentions.ts`, `components/chat/ChatSessionsRail.tsx`, `components/chat/ChatHeader.tsx`, `components/chat/ChatThreadSkeleton.tsx`. Tests: `agent-chat/sessions/useChatSessions.test.tsx`, `components/chat/hooks/useMentions.submit-veto.test.tsx`, `components/chat/__tests__/ChatSessionLoading.test.tsx` |
