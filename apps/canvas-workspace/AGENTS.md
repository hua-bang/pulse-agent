...
| AI Chat loading states | Two independent flags, do not conflate: `sessionsLoading` = the session LIST is being fetched (rail spinner in `ChatSessionsRail.tsx`, panel dropdown in `ChatHeader.tsx`); `sessionLoading` = THIS conversation's messages are being fetched (`ChatThreadSkeleton.tsx` in place of the thread, via `ChatView` → `ChatMessages`). Both live in `hooks/useChatSessions.ts`. Every thread-replacing IPC (`getHistory`, `loadSession`, `loadCrossWorkspaceSession`) MUST go through `runThreadFetch`: it owns the flag AND the monotonic request token that drops superseded responses — without it two quick session picks let the slower one overwrite the session the user actually chose, and `handleNewSession` must keep bumping that token so an abandoned load can't repaint a blank new chat. `sessionLoading` is seeded TRUE at mount (effects run after first paint, so a false seed flashes the empty state); `skipInitialHistory: true` therefore obliges the caller to call `handleLoadSession`. A third flag, `useChatStream`'s `loading`, means "the model is generating" — that one drives the three-dot `.chat-loading`, never the skeleton. Two rules keep the renderer and the main-side session pointer in agreement, and BOTH are load-bearing: (1) only one session SWITCH may be in flight — `loadSession`/`loadCrossWorkspaceSession`/`newSession` archive-and-promote main-side (`agent/service.ts` → `session-store.ts`), so overlapping switches leave the pointer at whichever finished LAST while the token paints whichever was clicked last, and the next turn then persists into a conversation the user is not looking at; `handleLoadSession`/`handleNewSession` therefore run through `runExclusiveSwitch`, which SERIALIZES them. Queue, never refuse: refusing was safe only while ChatPageBody remounted this hook per scope, and it now stays mounted, so a dropped cross-scope pick would leave the rail on the chosen session while the thread and the pointer stayed on the previous one. A switch superseded *while queued* is skipped — only the newest runs. `getHistory` is exempt — it activates and reads, never promotes — which is exactly the overlap the token still arbitrates. (2) Sending is vetoed while `sessionLoading` (mirror of the switches already blocked while streaming), and the veto wraps `sendMessage` in `useChatComposerState`, NOT the composer's submit path: quick actions arrive programmatically (Knowledge Chat Summarize → `useComposerRequest` → `handleQuickAction`) and DOM review submits its own prompt, both calling `sendMessage` directly. Tests: `hooks/useChatSessions.test.tsx`, `hooks/useChatComposerState.send-veto.test.tsx`, `__tests__/ChatSessionLoading.test.tsx` |
