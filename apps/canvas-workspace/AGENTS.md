...
| Full-page chat → dock hand-off | The full-page chat topbar (`chat/ChatPageBody.tsx`, shared by the AI Chat page and a scheduled task's chat page) has NO close button: leaving that page is only useful if the conversation survives it, so the one exit control opens the dock's Pulse AI tab AND calls `onExit` together — the page is exactly where `isDockChatTabEnabled` is false, so a dock open without the route change would target a hidden tab. `chat/dockChatHandoff.ts` picks the dock surface and must keep a scheduled scope's `taskId` (`openChat()` clears `scheduledChatTaskId` and would silently show the active workspace's chat instead). Esc and ⌘/Ctrl+Shift+L remain the keyboard exits from the AI Chat route. Tests: `chat/__tests__/dockChatHandoff.test.ts` |
