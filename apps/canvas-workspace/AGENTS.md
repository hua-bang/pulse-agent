...
| Coding agents in the Coding Agent node (roster, brand marks, launch flags) | Read `harness/knowledge/coding-agent-registry.md` before adding/removing an agent or changing per-agent launch flags. Key contract: `src/renderer/src/config/agentRegistry.ts`. Tests: `AgentNodeBody/__tests__/AgentPicker.test.tsx`, `AgentNodeBody/__tests__/AgentIcon.test.tsx`, `utils/__tests__/codingAgentCommand.test.ts`. |
