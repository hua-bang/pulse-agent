    ...
    <div className="note-outline" onMouseDown={(e) => e.stopPropagation()}>
      <div className="note-outline-head">
        ...
        <button
          ...
          onClick={onClose}
          title={t('noteOutline.closeOutline')}
          aria-label={t('noteOutline.closeOutline')}
        >
          ×
        </button>
      </div>
      {headings.length === 0 ? (
        <div className="note-outline-empty">{t('noteOutline.noHeadings')}</div>
      ) : (
          ...
          {headings.map((h, i) => (
            <li key={`${h.pos}-${i}`}>
              <button
                ...
                onClick={() => goTo(h.pos)}
                title={h.text || t('noteOutline.untitledHeading')}
              >
                {h.text || t('noteOutline.untitledHeading')}
              </button>
            </li>
          ))}
        </ul>
