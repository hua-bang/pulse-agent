...
describe('toolStreamState', () => {
  ...
  it('marks the oldest running same-name tool succeeded when result ids do not line up', () => {
    const tools: ToolCallStatus[] = [
      { id: 1, name: 'tavily', toolCallId: 'call-a', status: 'running' },
      { id: 2, name: 'tavily', toolCallId: 'call-b', status: 'running' },
    ];

    markToolResult(tools, {
      name: 'tavily',
    ...
    expect(tools[0]).toMatchObject({ status: 'succeeded', result: 'ok', inputStreaming: false });
    expect(tools[1].status).toBe('running');
  });

  it('preserves a failed tool result instead of presenting it as complete', () => {
    const tools: ToolCallStatus[] = [
      { id: 1, name: 'bash', toolCallId: 'call-a', status: 'running' },
    ];
  ...
  it('cancels any remaining running tools when the turn is stopped', () => {
    const tools: ToolCallStatus[] = [
      { id: 1, name: 'tavily', status: 'running' },
      { id: 2, name: 'visual_render', status: 'running', streamedContent: '<html></html>' },
    ];

    settleRunningTools(tools, 'cancelled', 'Operation cancelled');

    expect(tools.every(tool => tool.status === 'cancelled')).toBe(true);
    expect(tools[1].streamedDone).toBe(true);
  });

  it('fails unresolved tools when a turn ends without a result', () => {
    const tools: ToolCallStatus[] = [
      { id: 1, name: 'canvas_update_node', status: 'running' },
    ];

    settleRunningTools(tools, 'failed', 'No result received');
