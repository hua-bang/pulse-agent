...
/**
 * Small badge rendered on nodes the canvas agent created. Hover reveals
 * the agent's intent plus a "view in chat" affordance; click dispatches a
 * window-level event the chat panel can subscribe to in order to scroll
 * the originating message into view.
 *
 * Returns null when the node has no provenance, so callers can drop this
 * unconditionally into every render branch without per-type checks.
 */
const NodeSourceBadge = ({ node }: { node: CanvasNode }) => {
  if (!node.source || node.source.origin !== 'agent') return null;
  const { intent, createdAt, sourceNodeIds } = node.source;
  const handleClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    window.dispatchEvent(
      new CustomEvent('canvas:focus-conversation', {
        detail: { nodeId: node.id, source: node.source },
      }),
    );
  };
  const tooltipParts: string[] = ['Created by canvas agent'];
  if (intent) tooltipParts.push(intent);
  if (sourceNodeIds && sourceNodeIds.length > 0) {
    tooltipParts.push(
      `Derived from ${sourceNodeIds.length} node${sourceNodeIds.length === 1 ? '' : 's'}`,
    );
  }
  if (createdAt) {
    try {
      tooltipParts.push(new Date(createdAt).toLocaleString());
    } catch {
      // Ignore — bad timestamp shouldn't break the badge.
    }
  }
  return (
    <button
      type="button"
      className="node-ai-badge"
      onClick={handleClick}
      onMouseDown={(e) => e.stopPropagation()}
      title={tooltipParts.join(' · ')}
      aria-label="Node generated by canvas agent — click to view in chat"
    >
      <svg width="10" height="10" viewBox="0 0 12 12" fill="none" aria-hidden="true">
        <path
          d="M6 1.5l1.1 2.7L9.8 5.3 7.5 7l.6 2.8L6 8.4 3.9 9.8 4.5 7 2.2 5.3l2.7-1.1L6 1.5z"
          fill="currentColor"
        />
      </svg>
    </button>
