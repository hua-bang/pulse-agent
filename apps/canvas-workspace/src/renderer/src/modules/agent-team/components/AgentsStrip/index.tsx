    ...
    <div className="agent-team-agent-strip">
      {agents.length === 0 ? (
        ...
        return (
          <div
            key={agent.key}
            className={`agent-team-summary-agent agent-team-summary-agent--${agent.status}${
              selectedAgentKey === agent.key ? ' agent-team-summary-agent--selected' : ''
            }${selectedTaskOwnerKey === agent.key ? ' agent-team-summary-agent--task-owner' : ''}`}
          >
            <button
              type="button"
            ...
            >
              ...
              <span className={`agent-team-detail__status agent-team-detail__status--${agent.status}`}>
                {agentTeamStatusLabel(agent.status)}
                {agentSessionHealthSuffix(agent.sessionHealth)}
              </span>
              {!editable && (
            ...
            {editable && (
              <div className="agent-team-summary-agent__agent-select">
                ...
                <AgentTypeSelect
                  value={agentOptions.some((item) => item.id === agent.agentType)
                    ? agent.agentType!
                    : agentOptions[0].id}
                  options={agentOptions}
                  ariaLabel={`Coding agent for ${agent.name}`}
                  onChange={(id) => onChangeAgentType(agent.name, id)}
                />
              </div>
            )}
