...
import { mcpAppTabId, skillTabId } from '../../../../shared/dock/dock-tab-ids';
import type { ChatDeliveryReceipt } from '../../../chat';
import { DockPortProvider, type DockActions } from '../../../../shared/dockPort';
...
export const RightDockProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <RightDockContext.Provider value={value}>
      <DockPortBridge>{children}</DockPortBridge>
...
export function useRightDock(): DockActions {
  const {
    store,
    pinUrlReference,
    registerPinUrlReference,
...
const DockPortBridge = ({ children }: { children: ReactNode }) => {
  const context = useDockContext();
  const actions = useRightDock();
  const value = useMemo(() => ({
    actions,
    stateStore: context.store,
    chatHost: context.chatHost,
    terminalHost: context.terminalHost,
    mcpAppHosts: context.mcpAppHosts,
  }), [actions, context.store, context.chatHost, context.terminalHost, context.mcpAppHosts]);
  return <DockPortProvider value={value}>{children}</DockPortProvider>;
};
