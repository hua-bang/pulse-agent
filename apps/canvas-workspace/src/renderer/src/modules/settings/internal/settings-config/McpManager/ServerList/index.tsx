...
export const ServerList = ({ view, actions, t }: ServerListProps) => (
...
interface InheritedServerListProps {
  servers: CanvasMcpServer[];
...
export const InheritedServerList = ({
  ...
  onToggleExpanded,
  t,
}: InheritedServerListProps) => (
  <div className="cfg-inherited">
    <div className="cfg-inherited-header">
      <span className="cfg-inherited-title">{t('mcpConfig.inheritedTitle', { count: servers.length })}</span>
        ...
        return (
          <li key={server.name} className="cfg-list-entry">
            <div className={`cfg-list-item cfg-list-item--readonly${overridden ? ' cfg-list-item--shadowed' : ''}`}>
              <button type="button" className="cfg-expander" aria-expanded={isOpen} title={t(isOpen ? 'mcpConfig.collapseTools' : 'mcpConfig.expandTools')} onClick={() => onToggleExpanded(expandKey)}>
                {isOpen ? '▾' : '▸'}
              </button>
              <div className="cfg-list-main">
                <div className="cfg-list-title">
              ...
              {overridden && <span className="cfg-shadow-warn" title={t('mcpConfig.inheritedOverridden')}>⚠ {t('mcpConfig.inheritedOverridden')}</span>}
            </div>
            {isOpen && <div className="cfg-tools"><ToolsList health={statuses[server.name]} readOnly t={t} /></div>}
          </li>
        );
      })}
    </ul>
