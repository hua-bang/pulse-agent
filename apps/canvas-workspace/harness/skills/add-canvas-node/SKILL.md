...
3. **Renderer dispatch — read the fallthrough chain carefully, it is NOT one switch statement:**
   - `src/renderer/src/components/canvas/CanvasNodeView/index.tsx` short-circuits FIRST for `image`/`shape`/`reference`/`mindmap` (each returns its own dedicated body component directly).
