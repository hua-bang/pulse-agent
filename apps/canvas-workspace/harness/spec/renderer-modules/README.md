...
| Agent Team workspace | `modules/agent-team/components/AgentTeamFrame/index.tsx` ~907 lines; public model ~332 lines; controller ~190 lines; task, agent, inspector, gate, command, strip, DAG, and artifact viewer visuals are owner-local | lead dock, graph-workspace composition, selection state, and runtime mounts remain interleaved; artifact file-preview lifecycle now has an owned interface |
