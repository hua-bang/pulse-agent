...
| Agent Team workspace | `modules/agent-team/components/AgentTeamFrame/index.tsx` ~1463 lines; public model ~332 lines; controller ~190 lines; TaskDagCanvas, HumanGateCard, and TeamCommand are owner-local | task/agent/artifact detail visuals remain interleaved; command draft/submission, polling, IPC actions, snapshot projection, DAG layout, and gate UI now have owned interfaces |
