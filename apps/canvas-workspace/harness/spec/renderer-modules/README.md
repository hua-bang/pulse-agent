...
|---|---|---|
| Canvas document | Canvas composition ~439 lines; node gesture adapter ~48 lines; drawing gesture adapter ~105 lines; document adapter ~338 lines plus host feedback adapter ~85 lines and owner-local history/merge/command modules | persistence scheduling remains in the React adapter; node drag/resize/snap/render-order plus edge/shape/marquee mutual exclusion and post-commit selection now cross grouped interfaces; save retry, viewport restore, external-create feedback, creation/transfer commands, and transactions are separated |
