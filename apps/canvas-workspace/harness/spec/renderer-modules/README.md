...
| Agent Team workspace | `modules/agent-team/components/AgentTeamFrame/index.tsx` ~1373 lines; public model ~332 lines; controller ~190 lines; TaskDagCanvas, HumanGateCard, TeamCommand, and AgentsStrip are owner-local | agent/task/artifact detail visuals remain interleaved; command, polling, IPC actions, snapshot projection, DAG layout, gate UI, and agent summary strip now have owned interfaces |
