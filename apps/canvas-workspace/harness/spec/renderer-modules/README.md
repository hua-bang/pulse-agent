...
| Agent Team workspace | `modules/agent-team/components/AgentTeamFrame/index.tsx` ~996 lines; public model ~332 lines; controller ~190 lines; TaskDagCanvas, HumanGateCard, TeamCommand, AgentsStrip, AgentDetail, AgentInspector, and TaskDetail are owner-local | artifact detail remains interleaved; command, polling, IPC actions, snapshot projection, DAG layout, gate UI, agent summary, agent detail, and task detail now have owned interfaces |
