...
| Workspace node cards | shared interaction media CSS 71 lines; KnowledgeNodeCard CSS 143; CardShell CSS 52; NodeCardPreview CSS 218 lines | preview, interactive wrapper, and identity/footer/tags/actions all follow owner folders; only cross-filter/card hover, active, reduced-motion, and responsive rules remain shared |
| Workspace node filters | `NodeFilters/index.tsx` ~295 lines; owner CSS 219 lines | search, active scopes, workspace/type/tag popovers and counts now follow the owner-folder convention; remaining `NodeCards.css` is 266 lines of card shell/actions plus shared interaction media rules |
