...
| Agent Team workspace | `modules/agent-team/components/AgentTeamFrame/index.tsx` ~863 lines; public model ~332 lines; controller ~190 lines; task, agent, inspector, gate, command, strip, DAG, artifact viewer, and lead dock visuals are owner-local | graph-workspace composition, selection state, and runtime mounts remain interleaved; artifact preview and lead Coding Agent composition now have owned interfaces |
