...
|---|---|---|
| Canvas document | Canvas composition ~477 lines; node gesture adapter ~48 lines; document adapter ~338 lines plus host feedback adapter ~85 lines and owner-local history/merge/command modules | persistence scheduling remains in the React adapter; node drag/resize/snap/render-order and resize auto-size policy now cross one grouped interface; save retry, viewport restore, external-create feedback, creation/transfer commands, and transactions are separated |
