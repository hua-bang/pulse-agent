...
1. **整页 `/chat` 路由是否保留？**
   - 方案 A：保留整页作为「专注模式」，dock 作为「随手模式」，两者共享同一份会话状态。
   - 方案 B：移除整页，统一为 dock（更简洁，但失去全屏沉浸式聊天）。
2. **dock 默认停靠位置**：右侧（与现有 ChatPanel 一致，推荐）还是可左右切换？
3. **dock 开关是否跨视图记忆**：切到 Nodes 时是否保持上一视图的开/关与宽度？（推荐：记忆。）
