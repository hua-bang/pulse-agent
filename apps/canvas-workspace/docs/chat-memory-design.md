...
> 实现细节：插件的 `recall()` 只搜 `daily-log`，所以 `canvas_memory_recall` 在每个桶内额外用 `service.list()` + 关键词过滤捞出 explicit 记录（`record`/`promote` 写入的），否则纯工具模式下它们会"只写、召不回"。
...
canvas_memory_promote: { input: { content: string; kind?: 'rule'|'profile' } }                     // → global 桶 (Phase 1 暂不含 soul)
```
