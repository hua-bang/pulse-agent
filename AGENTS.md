...
- **TypeScript**: `strict:true` from root `tsconfig.json`. Keep strict ON. `apps/canvas-workspace` uses a standalone tsconfig — root changes do not reach it. `plugin-kit` typecheck hits TS6059 rootDir errors locally — default to `build` as the JS smoke check there. (`engine` and `acp` had the same class from cross-package source imports via a root alias; both fixed by dropping `rootDir` from their tsconfigs — `rootDir` is emit-layout config that `tsc --noEmit` and tsup do not need. Same fix likely applies here.)
